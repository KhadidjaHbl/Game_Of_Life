...
{	
	...
	for(i = 0; i < largeur; i++)
	{	
		// ici 512 represente (la hauteur de la matrice/nbr de slots) = (2048/4) nous avons entré la taille d'une maniere statique, 
		// donc, si vous voulez la changer veuillez remplacer ce nombre, et changer la taille dans la variable heuteur 
    ...
    // pour les threads de OpenMP
    int tailleop=taille/nthreads;
	...
	{	
		#pragma omp parallel shared(ptrCourant, ptrSuiv, temp, nthreads) private(tid, debutop, finop) num_threads(nthreads)
        {
            tid = omp_get_thread_num();	//devision des portions a l interieur de chaque slot 
            debutop = tid *tailleop + debut;
            finop = debutop + tailleop;
